...
const ScheduleScreen = ({ navigation }) => {
  ...
  async function insertTicket() {
    ...
    const { data, error: specificError } = await supabase
      .from("services")
      .select("service_id, parent_service_id, service_name, transaction_time")
      ...
      <View style={styles.card}>
        <View style={styles.queue}>
          <View style={styles.rowAlign}>
            <View
              style={disabledDays ? styles.dateContainer : styles.disabledCard}
            >
            ...
            <View style={styles.queueSection}>
              <TouchableOpacity
                style={[
                  styles.queueButton,
                  {
                    backgroundColor: selectedServices[dayIndex]
                      ? colors.baseBlue
                      : "#D3D3D3",
                  },
                ]}
        ...
        <View style={styles.servicesArea}>
          {mainservice.map((service, index) => {
            ...
            const queueColor =
              queueNumber < queueLimit * 0.5
                ? "green"
                : queueNumber < queueLimit * 0.9
                ? "orange"
                : "red";
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  styles.serviceBox,
                  {
                    backgroundColor: disabledDays[dayIndex]
                      ? colors.grayBorder
                      : isSelected
                      ? colors.darkBlue
                      : colors.lightBlue,
                    pointerEvents: disabledDays[dayIndex] ? "none" : "auto", // Corrected here
                  },
                ]}
              ...
              >
                ...
                <View style={styles.queueInfo}>
                  <Text style={[styles.queueNumber, { color: queueColor }]}>
                    {queueNumber}
                  </Text>
                  <Text style={[styles.queueNumber, { color: queueColor }]}>
                    /{queueLimit}
                ...
                {specificServices.map((service, index) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      {
                        backgroundColor:
                          selectedSpecificService === service
                            ? colors.darkBlue
                            : colors.white,
                      },
                    ]}
          ...
          <View style={styles.viewModalContent}>
            ...
            <View style={styles.viewModalBody}>
              <Text style={styles.viewTicketText}>
                You have successfully queued your ticket. View your ticket with
                the button below.
              </Text>
            </View>
            <View style={styles.modalLine}></View>
